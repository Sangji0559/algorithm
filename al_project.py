...
def find_max_volume(matrix,j):
    ...
    sorted_cols = np.argsort(np.linalg.norm(matrix, axis=0))[-50:]
    ...
    for k in range(20):
        ...
        if k == 0:
            ...
            max_index=sorted_cols[j]
        else:
            result_matrix = np.hstack((max_subset, result_matrix)) #if k == 1 else np.hstack((max_subset, result_matrix))
